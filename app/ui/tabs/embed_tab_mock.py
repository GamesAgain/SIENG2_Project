...
class MetadataDropWidget(QLabel):
    ...
    def dropEvent(self, event: QDropEvent):
        self.dragLeaveEvent(event) # Restore style
        if event.mimeData().hasUrls():
            for url in event.mimeData().urls():
                file_path = url.toLocalFile()
                ext = os.path.splitext(file_path)[1].lower()
...
class EmbedTabMockUp(QWidget):
    ...
    def __init__(self):
        ...
        self.guide_text = ""  # Storage for guide note text
    ...
    def _create_configurable_page(self):
        ...
        preview_scroll.setMinimumHeight(300)  # Increased for better visibility (Image + Stats)
        ...
        # Editor section with scroll area (Restored)
        ...
        # Reduced min height for editor to allow it to shrink if needed, relying on scroll
        editor_scroll.setMinimumHeight(120) 
        
        editor_container = QWidget()
        editor_layout = QVBoxLayout(editor_container)
        editor_layout.setContentsMargins(4, 4, 4, 4)
        editor_layout.setSpacing(3)  # Reduced from 6 for space optimization
        ...
        # Set initial sizes (Favor Preview 500px : Editor 300px)
        splitter.setSizes([500, 300])
        splitter.setStretchFactor(0, 1)
        splitter.setStretchFactor(1, 0) # Give priority to Preview expansion
    ...
    def _create_pipeline_tab(self, tab_type):
        ...
        if tab_type == "embed":
            ...
            self.btn_guide_note = QPushButton("GuideNote")
    ...
    def simulate_embedding_process(self):
        ...
        if hasattr(self, 'progress_bar') and hasattr(self, 'status_label'):
            ...
            # Simulate loading
            for i in range(101):
                QTimer.singleShot(i * 10, lambda val=i: self._update_progress(val))
        else:
    ...
    def open_guide_editor(self):
        """Open popup dialog to edit guide text"""
        dialog = TextEditorDialog(self.guide_text, self)
        dialog.setWindowTitle("Edit Guide Note")
        if dialog.exec():
            self.guide_text = dialog.get_text()
    ...
    def on_mode_changed(self):
        ...
        # Toggle Standalone Progress UI visibility
        if hasattr(self, 'standalone_progress_bar'):
            self.standalone_progress_bar.setVisible(not is_configurable)
        if hasattr(self, 'standalone_status_label'):
    ...
    def on_technique_changed(self):
        ...
        # Update Payload Group Title
        if hasattr(self, 'payload_main_group'):
            if is_metadata:
    ...
    def _on_metadata_file_dropped(self, file_path):
        """Handle file dropped on metadata preview widget"""
        self.current_image_path = file_path
        self.carrier_edit.setText(file_path)
        self._load_preview_generic_left(file_path)
        self.update_payload_ui_for_metadata(file_path)
        if hasattr(self, 'read_metadata_and_fill'):
            self.read_metadata_and_fill(file_path)
    ...
    def update_payload_ui_for_metadata(self, file_path):
        ...
        if ext == '.png':
            self._build_png_editor(container)
        elif ext == '.mp3':
            self._build_mp3_editor(container)
        elif ext in ['.jpg', '.jpeg']:
            self._build_jpeg_editor(container)
        else:
            label = QLabel("Unsupported file type for metadata editing.")
            label.setStyleSheet("color: #aaa; font-style: italic; margin-top: 20px;")
            label.setAlignment(Qt.AlignmentFlag.AlignHCenter)
            container.layout().addWidget(label)

    def _build_png_editor(self, container):
        """Build a Table Editor for PNG Text Chunks (iTXt)."""
        # Header
        header = QLabel("PNG Text Chunks (iTXt/tEXt)")
        header.setStyleSheet("font-weight: bold; color: #3daee9; font-size: 11pt; margin-bottom: 5px;")
        container.layout().addWidget(header)
        
        # Table
        self.png_table = QTableWidget()
        self.png_table.setColumnCount(3)
        self.png_table.setHorizontalHeaderLabels(["Keyword", "Text Chunk Data", "Action"])
        self.png_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        self.png_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self.png_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed)
        self.png_table.setColumnWidth(2, 60)
        self.png_table.verticalHeader().setVisible(False)
        self.png_table.setAlternatingRowColors(True)
        self.png_table.setStyleSheet("""
            QTableWidget {
        ...
        """)
        container.layout().addWidget(self.png_table)
        ...
        """)
        btn_add.clicked.connect(self._add_empty_png_row)
        container.layout().addWidget(btn_add)
        
    def _add_empty_png_row(self):
        self._add_png_row("", "")
        
    def _add_png_row(self, key, value):
        row = self.png_table.rowCount()
        self.png_table.insertRow(row)
        
        # Key Item
        item_key = QTableWidgetItem(key)
        self.png_table.setItem(row, 0, item_key)
        
        # Value Item
        item_val = QTableWidgetItem(value)
        self.png_table.setItem(row, 1, item_val)
        ...
        """)
        btn_del.clicked.connect(lambda: self.png_table.removeRow(self.png_table.row(btn_del)))
        ...
        self.png_table.setCellWidget(row, 2, widget)

    def _build_mp3_editor(self, container):
        """Build a Form Editor for MP3 ID3 Tags."""
        scroll = QScrollArea()
        ...
        tags = [
            ...
            ("Year", "TYER"), ("Genre", "TCON"), ("Comment", "COMM"),
            ("Track", "TRCK"), ("Copyright", "TCOP")
        ]
        
        self.mp3_fields = {}
        
        for label, tag_id in tags:
            lbl = QLabel(label)
            ...
            inp.setPlaceholderText(f"ID3 Tag: {tag_id}")
            self.mp3_fields[tag_id] = inp
            form.addRow(lbl, inp)
            
        scroll.setWidget(content)
        container.layout().addWidget(scroll)

    def _build_jpeg_editor(self, container):
        """Build a Form Editor for JPEG EXIF Tags."""
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setFrameShape(QScrollArea.Shape.NoFrame)
        ...
        for label, tag_key in tags:
            lbl = QLabel(label)
            lbl.setStyleSheet("color: #aaa;")
            
            if "Comment" in label or "Description" in label:
                inp = QTextEdit()
                inp.setMinimumHeight(60)
                inp.setMaximumHeight(80)
            else:
    ...
    def read_metadata_and_fill(self, file_path):
        ...
        # Helper to set value in QLineEdit/QTextEdit
        def set_field_val(field_dict, key, value):
            if key in field_dict and value:
                widget = field_dict[key]
                val_str = str(value).strip()
                if isinstance(widget, (QLineEdit, QTextEdit)):
                    widget.setText(val_str)

        # 1. PNG Metadata (Pillow)
        if ext == '.png' and Image:
            try:
                # Clear existing rows first (except the empty one we might want to keep, but better clear all)
                self.png_table.setRowCount(0)
                
                with Image.open(file_path) as img:
                    info = img.text  # Use img.text for iTXt/tEXt chunks
                    for key, value in info.items():
                        self._add_png_row(key, str(value))
                        
            except Exception as e:
                print(f"Error reading PNG metadata: {e}")

        # 2. MP3 Metadata (Mutagen)
    ...
    def _build_execution_group(self, button_text):
        ...
        self.btn_exec.setMinimumHeight(35)  # Reduced from 45
    ...
    def _create_metadata_payload_page(self):
        ...
        self.preview_label_meta_left = MetadataDropWidget()
        self.preview_label_meta_left.fileDropped.connect(self._on_metadata_file_dropped)
        layout.addWidget(self.preview_label_meta_left, 1)
        
        # Add info label for file name/size
        self.preview_info_label_meta_left = QLabel("")
        self.preview_info_label_meta_left.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.preview_info_label_meta_left.setStyleSheet("color: #e0e0e0; font-size: 9pt;")
        self.preview_info_label_meta_left.hide()
    ...
    def _on_run_embed(self) -> None:
        ...
        if "Metadata" in self.tech_combo.currentText():
            self._run_metadata_embedding()
            return
